...
#HW Assignment 2: Lexical Scoping by ASteen
...
cacheSolve <- function(x, ...) {
    ...
    return(m) #this will return the inverse matrix of x
  }

#testing the matrix 
tester1 <- matrix(rnorm(16),nrow=4,ncol=4)
tester1
p1 <-solve(tester1)
p2 <- makeCacheMatrix(tester1)
p2$get()
p2$getsolve()
p3<- cacheSolve(p2)
p3<- cacheSolve(p2)

#test 2 of the matrix 
tester2 <-martix(rnorm(64),nrow=4, ncol=4)
tester2
